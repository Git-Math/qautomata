...
fn setup(mut commands: Commands) {
    ...
    let mut grid_entities: GridEntities = GridEntities([[Entity::from_raw(0); GRID_SIZE]; GRID_SIZE]);
...
fn step(mut commands: Commands, grid_entities: Res<GridEntities>) {
    commands.entity(grid_entities.0[3][0]).insert_bundle(SpriteBundle {
        sprite: Sprite {
...
fn coordinates_to_vec(x: f32, y: f32) -> Vec3 {
    Vec3::new(y * CELL_WITH_SPACE_SIZE + SHIFT_X, x * CELL_WITH_SPACE_SIZE * -1.0 + SHIFT_Y, 0.0)
}
