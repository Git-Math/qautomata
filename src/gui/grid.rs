...
fn step(mut commands: Commands, grid_entities: Res<GridEntities>, mut universe: ResMut<Universe>) {
    ...
    //println!("Living cells in combined state: {}", universe.combined_state.len());
    ...
    if universe.state.len() > 128 {
        ...
        for x in 0..GRID_SIZE {
            for y in 0..GRID_SIZE {
                let color: Color;
                match universe.state[0].living_cells.get(&Coordinates {
                    x: x as i32,
                    y: y as i32,
                }) {
                    Some(_) => color = Color::rgb(0.0, 0.0, 0.0),
                    None => color = DEAD_CELL_COLOR,
                }
        ...
        for x in 0..GRID_SIZE {
            for y in 0..GRID_SIZE {
                let color: Color;
                match universe.combined_state.get(&Coordinates {
                    x: x as i32,
                    y: y as i32,
                }) {
                    Some(p) => {
                        let rgb = 1.0 - *p as f32;
    ...
    .insert_bundle(SpriteBundle {
        ...
        transform: Transform {
            translation: coordinates_to_vec(10 as f32, 15 as f32),
            scale: Vec3::new(1.0, 1.0, 1.0),
            ..default()
        },
        ..default()
